...
public void Event_RoundEnd_72_FreeRespawn(Event event, const char[] name, bool dontBroadcast) {
     for(int i = 1; i <= MaxClients; i++) {
          E72_NullifyClientRespawnData(i);
...
public void Event_PlayerDeath_72_FreeRespawn(Event event, const char[] name, bool dontBroadcast) {
     ...
     g_Effect72_HudTimer[client] = CreateTimer(0.5, E72_RespawnHudTick, client, TIMER_REPEAT);
     g_Effect72_RespawnTimer[client] = CreateTimer(E72_RESPAWN_TIME, E72_FreeRespawnPlayer, client);
     CreateTimer(0.1, E72_RespawnPlayerGhost, client);
}

public Action E72_RespawnPlayerGhost(Handle timer, int client) {
...
public Action E72_RespawnHudTick(Handle timer, int client) {
...
public Action E72_FreeRespawnPlayer(Handle timer, int client) {
     if(!IsClientInGame(client)) {
          return Plugin_Handled;
     }
     ...
     E72_NullifyClientRespawnData(client);
     return Plugin_Handled;
}

public void E72_NullifyClientRespawnData(int client) {
